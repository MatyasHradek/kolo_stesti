...
const segments = ["Výhra", "Zkus to znovu", "Sleva 10%", "Výhra 2x", "Nic", "Sleva 20%"];
const colors = ["#FF6347", "#FFD700", "#ADFF2F", "#00CED1", "#FF69B4", "#9370DB"];
...
function spinWheel() {
  ...
  function animate(now) {
    ...
    if (progress < 1) {
      ...
      const selected = Math.floor((segments.length - (currentAngle % 360) / segAngle) % segments.length);
      result.textContent = `Výsledek: ${segments[selected]}`;
    }
  }
